import React from 'react';
import { Table, Tag, Typography, Empty, Badge } from 'antd';
import { useQuery } from '@tanstack/react-query';
import { getUserPaymentsByUser } from '../../api/userPayments';
import { useAuthStore } from '../../store/authStore';
import { formatCurrency } from '../../utils/helpers';
import { useTranslation } from 'react-i18next';
import type { UserPayment } from '../../types';
import dayjs from 'dayjs';

const { Title } = Typography;

const MyAdminPaymentsPage: React.FC = () => {
  const { user } = useAuthStore();
  const { t } = useTranslation();

  const { data: payments = [], isLoading } = useQuery({
    queryKey: ['my-admin-payments', user?.id],
    queryFn: () => getUserPaymentsByUser(user!.id),
    enabled: !!user?.id,
  });

  const columns = [
    {
      title: t('common.date'),
      dataIndex: 'date',
      key: 'date',
      width: 160,
      render: (d: string) => dayjs(d).format('DD.MM.YYYY HH:mm'),
      sorter: (a: UserPayment, b: UserPayment) => dayjs(a.date).unix() - dayjs(b.date).unix(),
      defaultSortOrder: 'descend' as const,
    },
    {
      title: t('common.invoice'),
      dataIndex: 'userInvoiceId',
      key: 'invoice',
      width: 100,
      render: (id?: number) => id ? `#${id}` : '—',
    },
    {
      title: t('payments.payment_method'),
      dataIndex: 'paymentMethod',
      key: 'method',
      width: 130,
      render: (v: string) => <Tag>{v}</Tag>,
    },
    {
      title: t('common.amount'),
      dataIndex: 'amount',
      key: 'amount',
      width: 140,
      render: (v: number) => formatCurrency(v),
    },
    {
      title: t('common.status'),
      dataIndex: 'accepted',
      key: 'status',
      width: 110,
      render: (v: boolean) => v
        ? <Badge status="success" text={t('user_payments.accepted_status')} />
        : <Badge status="warning" text={t('user_payments.pending_status')} />,
    },
  ];

  const total = payments.reduce((s, p) => s + p.amount, 0);

  return (
    <>
      <Title level={4}>{t('payments.my_admin_payments_title')}</Title>
      <Table
        rowKey="id"
        dataSource={payments}
        columns={columns}
        loading={isLoading}
        size="small"
        pagination={{ pageSize: 20, showSizeChanger: true }}
        locale={{ emptyText: <Empty description={t('payments.no_payments')} /> }}
        summary={() => payments.length > 0 ? (
          <Table.Summary.Row>
            <Table.Summary.Cell index={0} colSpan={4} align="right">
              <Typography.Text strong>{t('common.total')}</Typography.Text>
            </Table.Summary.Cell>
            <Table.Summary.Cell index={1}>
              <Typography.Text strong>{formatCurrency(total)}</Typography.Text>
            </Table.Summary.Cell>
          </Table.Summary.Row>
        ) : null}
      />
    </>
  );
};

export default MyAdminPaymentsPage;
