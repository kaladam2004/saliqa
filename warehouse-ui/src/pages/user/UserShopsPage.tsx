...
const UserShopsPage: React.FC = () => {
  ...
  const { data: allShops = [], isLoading } = useQuery({ queryKey: ['shops'], queryFn: getShops });
  const shops = allShops.filter(s => s.shopkeeper?.id === user?.id);

  const saveMutation = useMutation({
    mutationFn: (data: ShopRequest) => editing
      ? updateShop(editing.id, { ...data, shopkeeperId: editing.shopkeeper?.id, test: editing.test })
      : createShop({ ...data, shopkeeperId: user?.id, test: false }),
    onSuccess: () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, r: Shop) => (
        <Space>
          <Button icon={<EditOutlined />} size="small" onClick={() => { setEditing(r); form.setFieldsValue(r); setModal(true); }} />
  ...
  return (
    <>
      <PageHeader title={t('shops.my_shops')} onAdd={() => { setEditing(null); form.resetFields(); setModal(true); }} />
